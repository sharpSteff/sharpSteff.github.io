...
There's an honest trade-off here worth calling out: `AutoAdminLogon` stores the password **in plain text** in the registry. For a locked-down, single-purpose CI node on an
...
> One more detail: Windows can be configured to auto-logon only a fixed number of times via `AutoLogonCount`. Removing that value keeps auto-logon permanent, which is what you want
...
fixed coordinates. The agent launch script forces a known resolution (e.g. 1920x1080) via
...
runs over SSH as one account, but Jenkins jobs run as the *agent* account. A `--global` install lands in the provisioning user's profile and is invisible to the agent. A
